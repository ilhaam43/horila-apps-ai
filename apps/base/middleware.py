...
class PerformanceOptimizationMiddleware:
    """Middleware untuk optimasi performa aplikasi"""
    ...
    def __call__(self, request):
        ...
        # Log slow requests (> 2 detik)
        if response_time > 2.0 and not settings.DEBUG:
            import logging
        ...
        if request.path.startswith('/static/'):
            ...
            if not response.get('ETag'):
                response['ETag'] = f'"static-{hash(request.path)}"'
